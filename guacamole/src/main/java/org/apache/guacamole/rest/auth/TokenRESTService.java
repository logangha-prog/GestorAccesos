...
public class TokenRESTService {
    ...
    @DELETE
    @Path("/kill/{user}")
    public void killUser(@PathParam("user") String userToken)
    		throws GuacamoleException {
    	Map<String, String> userTokens =  authenticationService.getUserToken(userToken);
    ...
    @GET
    @Path("/cancel/{organization}")
    public Map<String, String> getAllTokens(@PathParam("organization") String organizationToken)
    		throws GuacamoleException {
    	return authenticationService.getAllTokens(organizationToken);
    }

    @GET
    @Path("/user/cancel/{user}")
    public Map<String, String> getUserToken(@PathParam("user") String userToken)
    		throws GuacamoleException {
    	return authenticationService.getUserToken(userToken);
    }

    @GET
    @Path("/allusers")
    public Map<String, Map<String, List<String>>> getAllUsers() throws Exception {
        Map<String, Map<String, List<String>>> data = null;
        try {
            data = authenticationService.getAllUsers();
            for (Map.Entry<String,Map<String, List<String>>> entry : data.entrySet())  {
                 logger.info("Access Type[" + entry.getKey() + "]");
                 Map<String, List<String>> dataOrg = entry.getValue();
                 for (Map.Entry<String, List<String>> entryOrg : dataOrg.entrySet())  {
                      logger.info("Organization[" + entryOrg.getKey() + "]");
                      List<String> list = entryOrg.getValue();
    ...
    public Map<String, LoggedUser> getLoggedUsers() throws GuacamoleException {
        return authenticationService.getLoggedUsers();
    }
}
