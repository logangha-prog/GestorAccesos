...
angular.module('index').controller('indexController', ['$scope', '$injector',
    ...
    $scope.reAuthenticate = function reAuthenticate() {
     ...
     // Unified helper function to display login screen for authentication errors
    function showLoginScreen(error, helpText, acceptedCredentials) {
        $scope.applicationState = ApplicationState.AWAITING_CREDENTIALS;
        $scope.page.title = 'APP.NAME';
        $scope.page.bodyClassName = '';
        
        $scope.loginHelpText = helpText !== undefined ? helpText : null;
        $scope.acceptedCredentials = acceptedCredentials !== undefined ? acceptedCredentials : {};
    ...
    $scope.$on('guacIntentDenied', function intentDenied(event, parameters, error) {
        showLoginScreen(error);
    });
    ...
    $scope.$on('guacInsufficientCredentials', function loginInsufficient(event, parameters, error) {
        showLoginScreen(error, error.translatableMessage, parameters);
    });
