...
angular.module('auth').factory('authenticationService', ['$injector',
    ...
    service.authenticate = function authenticate(parameters) {
        ...
        ['catch'](requestService.createErrorCallback(function authenticationFailed(error) {
            ...
            else if (error.type === Error.Type.INTENT_UNALLOWED)
                $rootScope.$broadcast('guacIntentDenied', parameters, error);
    ...
    service.revokeTokens = function revokeTokens(tokens) {
        dataSourceService.apply(
            service.revokeToken,
            tokens
        )
        .then(function removedAccessRetrieved(removedAccess) {
        }, requestService.DIE);
    }
