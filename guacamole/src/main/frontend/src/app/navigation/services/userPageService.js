...
angular.module('navigation').factory('userPageService', ['$injector',
     ...
     */
    var generateHomePage = function generateHomePage(rootGroups, permissions, user) {

        var settingsPages = generateSettingsPages(permissions, user);
    ...
    service.getHomePage = function getHomePage() {
        ...
        var getUser = dataSourceService.apply(
            userService.getUser,
            authenticationService.getAvailableDataSources(),
            authenticationService.getCurrentUsername()
        );
        ...
        .then(function rootConnectionGroupsPermissionsRetrieved(data) {
            deferred.resolve(generateHomePage(data.rootGroups, data.permissionsSets, data.user));
        }, requestService.DIE);
     ...
     */
    var generateSettingsPages = function generateSettingsPages(permissionSets, user) {
        ...
        var canManageLoggedUsers = [];
        var canManageNECQClient = [];

        var dataSourceUser = authenticationService.getDataSource();
        if (user[dataSourceUser] === undefined 
           || user[dataSourceUser].attributes === undefined
           || user[dataSourceUser].attributes[User.Attributes.ORGANIZATION] === undefined) {
            var dataSources = Object.keys(user);
            var lenghtList = dataSources.length;
            for (i = 0; i < lenghtList; i++) {
                var ds = dataSources[i];
                if (user[ds] !== undefined
                    && user[ds].attributes
                    && user[ds].attributes[User.Attributes.ORGANIZATION] !== undefined) {
                    dataSourceUser = ds;
                    break;
                }
            }
        ...
        var role = null;
        var org = null;
        if (user[dataSourceUser] && user[dataSourceUser].attributes) {
    	    role = user[dataSourceUser].attributes[User.Attributes.ORGANIZATIONAL_ROLE];
	        org = user[dataSourceUser].attributes[User.Attributes.ORGANIZATION];
        }
        ...
        angular.forEach(authenticationService.getAvailableDataSources(), function inspectPermissions(dataSource) {
            ...
            if (!role || role != 'Finalizar-Sesiones') {
                // Determine whether the current user needs access to the user management UI
                if (
                    // System permissions
           ...
           if (!role || role != 'Finalizar-Sesiones') {
            ...
            // Determine whether the current user needs access to the logged and NEC user management UI
            if (
                // System permissions
                   PermissionSet.hasSystemPermission(permissions, PermissionSet.SystemPermissionType.ADMINISTER)
            ) {
                canManageLoggedUsers.push(dataSource);
                // Determine whether the current user needs access to the NEC qclient UI
                if (org && (org.indexOf('CNS') != -1 || org.indexOf('CNA') != -1)) {
            	    canManageNECQClient.push(dataSource);
                }
            }
        ...
        // Add link to Logged Users (only admin)
        if (canManageLoggedUsers.length) {
            pages.push(new PageDefinition({
                name : 'USER_MENU.ACTION_MANAGE_LOGGED',
                url  : '/settings/loggedUsers'
            }));
        }

        // Add link to NEC qclient (only Finalizarcns4 admin)
        if (canManageNECQClient.length) {
       	     pages.push(new PageDefinition({
                 name : 'USER_MENU.ACTION_MANAGE_NEC',
                 url  : '/settings/necSessions'
             }));
        }
     ...
     */
    var generateMainPages = function generateMainPages(rootGroups, permissions, user) {
        
        var pages = [];

        // Get home page and settings pages
        var homePage = generateHomePage(rootGroups, permissions, user);
        var settingsPages = generateSettingsPages(permissions, user);
    ...
    service.getMainPages = function getMainPages() {
        ...
        if (rootGroups && permissions && user)
             deferred.resolve(generateMainPages(rootGroups, permissions, user));
        };
